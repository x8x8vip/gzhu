...
var MAC={
	...
	'History': {
		...
		'List': function($id){
			...
			if ($(".history_box").is(":hidden")) {
			   MAC.History.FlagHide();
			   MAC.History.Show();
			   $('.footer').after('<div class="history_5nj"></div>');
			} else {
			   MAC.History.FlagHide();
			}
		...
		'Create': function($id){
			...
			html = '<dl class="drop-box history_box" id="history_box">';
			html +='<dt><span>观看记录</span><a target="_self" href="javascript:void(0)" onclick="MAC.History.Clear();">清空</a></dt>';
			if(jsondata.length > 0){
				for($i=0; $i<jsondata.length; $i++){
					...
					html +='<span>['+jsondata[$i].playname+']</span><a href="'+jsondata[$i].playlink+'" class="hx_title">'+jsondata[$i].name+'</a></dd>';
				}
			}else{
			...
			var h = $('#'+$id).height();
			var position = $('#'+$id).position();
			$('#history_box').css({'top':(position.top+h)});
		},	
		'Insert': function(name,playlink,typename,typelink,pic,playname){
			...
			if(jsondata != undefined){
				...
				if(!playlink){ playlink = document.URL; }
				jsonstr = '{video:[{"name":"'+name+'","playlink":"'+playlink+'","typename":"'+typename+'","typelink":"'+typelink+'","pic":"'+pic+'","playname":"'+playname+'"},';
				for($i=0; $i<=this.Limit; $i++){
					if(this.Json[$i]){
						jsonstr += '{"name":"'+this.Json[$i].name+'","playlink":"'+this.Json[$i].playlink+'","typename":"'+this.Json[$i].typename+'","typelink":"'+this.Json[$i].typelink+'","pic":"'+this.Json[$i].pic+'","playname":"'+this.Json[$i].playname+'"},';
					}else{
						break;
					}
				}
	...
	'Score': {
		...
		'Show':function($f,$tab,$id){
			var str = '';
			if($f==1){
								str += '<div id="star_5nj" class="star"  style=" border:0; margin:0 10px;"><ul style=" margin:0"><li class="star_current"></li>';
				for(var i=1;i<=10;i++){ str += '<li><a value="'+i+'" class="star_'+i+'">'+i+'</a></li>'; }
				str += '</ul>';
				str +='<span  style="margin:2px 8px 0 0px;" id="star_hover"></span><p style="margin:-5px -30px 0 0px;"><span id="star_shi">0</span><span id="star_ge">.0</span></p></div>';
			}
			else{
...
/*						$("#scoresend").click(function(){
		...
		'View':function($r){
			...
			document.getElementById("star_5nj").title=("总分" +$r.scoreall+"，已有" +$r.scorenum+"人评分");
		}
	},
	'Digg': {
